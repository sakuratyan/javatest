...
public class Dielectic extends Material {
    ...
    public boolean scatter(Ray r, HitRecord rec) {
        ...
        Vec3 reflected = reflect(r.direction(), rec.normal);//反射向量
        ...
        if (refract(r.direction(), outward_normal, ni_over_nt, rec)) {
            //发生了折射 计算反射系数
        ...
        if (Math.random() < reflect_prob) {
            rec.ref2Ray = new Ray(rec.p, reflected);
        }
        else {
            rec.ref2Ray = new Ray(rec.p, rec.refractedVec3);
        }
        return true;
    }
     ...
     * @return 是否有折射
     */
    public boolean refract(Vec3 v, Vec3 n, float nt, HitRecord rec){
        Vec3 uv = v.normalize();
        float cos_a1 = -1.0f * uv.dot(n);
        float temp = 1.0f - nt*nt*(1.0f-cos_a1*cos_a1);
        if(temp > 0.0f){
            rec.refractedVec3 = uv.Scale(nt).Add(n.Scale((float)(nt*cos_a1 - Math.sqrt(temp))));
            return true;
        }
        else {
            return false;
